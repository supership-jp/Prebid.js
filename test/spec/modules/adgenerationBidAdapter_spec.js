...
describe('AdgenerationAdapter', function () {
  ...
  const ENDPOINT = ['https://api-test.scaleout.jp/adgen/prebid', 'https://d.socdm.com/adgen/prebid'];
  ...
  describe('buildRequests', function () {
    ...
    it('sends bid request to ENDPOINT via POST', function () {
      ...
      expect(url.origin + url.pathname).to.equal(ENDPOINT[1]);
    ...
    it('sends bid request to debug ENDPOINT via POST', function () {
      ...
      const copyBidRequests = JSON.parse(JSON.stringify(bidRequests[0]));
      copyBidRequests.params.debug = true;
      const request = spec.buildRequests(bidRequests, bidderRequest)[0];
      const url = new URL(request.url);
      expect(url.origin + url.pathname).to.equal(ENDPOINT[1]);
      expect(url.searchParams.get('posall')).equal('SSPLOC');
    ...
    it('should attache params to the banner request', function () {
      const request = spec.buildRequests(bidRequests, bidderRequest)[0];
      // ignore random values
      delete request.data.ortb.id;
      expect(request.data).to.deep.equal(buildRequests.bannerRquests);
    });
    ...
    it('should attache params to the native request', function () {
      ...
      // ignore random values
      delete request.data.ortb.id;
      expect(request.data).to.deep.equal(buildRequests.nativeRequests);
    });
  ...
  describe('interpretResponse', function () {
    ...
    bidResponses.emptyAdomain = {};
    bidResponses.emptyAdomain.banner = JSON.parse(JSON.stringify(bidResponses.normal.banner));
    bidResponses.emptyAdomain.banner.adomain = [];
    bidResponses.emptyAdomain.native = JSON.parse(JSON.stringify(bidResponses.normal.native));
    bidResponses.emptyAdomain.native.adomain = [];

    bidResponses.noAdomain = {};
    bidResponses.noAdomain.banner = JSON.parse(JSON.stringify(bidResponses.normal.banner));
    delete bidResponses.noAdomain.banner.adomain;
    bidResponses.noAdomain.native = JSON.parse(JSON.stringify(bidResponses.normal.native));
    delete bidResponses.noAdomain.native.adomain;
